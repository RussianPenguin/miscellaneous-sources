...
void CMenuButton::TrackPopup(CWnd* pBar, CWnd* pWndSentCmd)
{
	...
	if (m_pSubMenu == pMenuBar->m_pWindowMenu)
		_bWindowMenuSendCmd = TRUE;
	else
		_bWindowMenuSendCmd = FALSE;

	pMenuBar->m_arrDepth=1;
	if(pMenuBar->m_arrDepth>=pMenuBar->m_arrSel.GetSize()) 
	{
		pMenuBar->m_arrSel.SetSize(pMenuBar->m_arrDepth+5);
		pMenuBar->m_arrNSel.SetSize(pMenuBar->m_arrDepth+5);
	}
	pMenuBar->m_arrSel[pMenuBar->m_arrDepth]=pMenuBar->m_pMenu->GetSubMenu(pMenuBar->GetCurIndex());
	pMenuBar->m_arrNSel[pMenuBar->m_arrDepth]=pMenuBar->GetCurIndex();
#ifdef DEBUG
	CString str;
	HMENU tmp;
	tmp=(pMenuBar->m_arrSel[pMenuBar->m_arrDepth]!=NULL)?
		pMenuBar->m_arrSel[pMenuBar->m_arrDepth]->m_hMenu:0;
	str.Format("Level: %d - hMenu: %x - Index: %d \n",pMenuBar->m_arrDepth,
		tmp,
		pMenuBar->m_arrNSel[pMenuBar->m_arrDepth]);
	TRACE (str);
#endif

	TrackPopupMenuEx(m_pSubMenu->m_hMenu,fuFlags,
...
void CMenuBar::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
...
void CMenuBar::MeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
...
CSubclassWnd* CMDIFrameHook::GetMainWndHook()
{
	return GetHookMap().Lookup(AfxGetMainWnd()->GetSafeHwnd());
}

void CMenuBar::RefreshPopup(UINT nIndex)
{
	ASSERT(nIndex>=0);

	RefreshMenuPainting(m_pMenu,nIndex);

	m_arrItem[nIndex]->TrackPopup(this,m_pParent);
}

BOOL CMenuBar::GetItemRect(int pos, CRect &rect)
{
	if(pos<0 || pos>=m_arrItem.GetSize()) return FALSE;
	rect=m_arrItem[pos]->GetRect();
	return TRUE;
}

void CMenuBar::UpdateMenuRefresh(UINT iCmd)
{
	int a,b;
	a=0;
	m_nMaxUse=0;
	m_nMinUse=0xffff;
	b=UpdateMenuUse(m_pMenu,iCmd,a,m_nMinUse,m_nMaxUse);
	if (b)
	{
		m_nTotUse=b;
		m_nMenuCmd=a;
	}
	a=((m_nMaxUse-m_nMinUse)*(((CWinAppEx*)AfxGetApp())->GetMenuParameter()))/100;
	if (a<0) a=10;
	UpdateDrawnGrayed(m_pMenu, m_nTotUse/m_nMenuCmd, a);
}

int CMenuBar::UpdateMenuUse(CMenuEx *pMenu, UINT iCmd, int& nTmpMenu,int& nMinUse,int& nMaxUse)
{
	int i,TotUse=0,n;
	UINT uId;
	if(!pMenu) return 0;
	...
	for (i=0;i<n;i++)
	{
		pTmpMenu=pMenu->GetSubMenu(i);
		if (pTmpMenu) 
		{
			TotUse+=UpdateMenuUse(pTmpMenu,iCmd,nTmpMenu, nMinUse,nMaxUse);
		}
		else
		{
			uId=pMenu->GetMenuItemID(i);
			if (uId==ID_SEPARATOR) continue;
			UINT t=pMenu->GetFlagsEx(i);
			int tt=t&0xffff;
			if (tt!=0 && tt!=0xffff)
			{
				if (tt<nMinUse) nMinUse=tt;
				if (tt>nMaxUse) nMaxUse=tt;
			}
			if (uId==iCmd)
			{
				if ((t&0xffff)!=0xffff)
					t=((DWORD)(IncMenuUsage(pMenu, (UINT) i, (WORD) t&0xffff))) | (t&0xffff0000);
				pMenu->SetFlagsEx(i,t);
			}
			nTmpMenu++;
			if ((t&0xffff)!=0xffff && (t&0xffff)!=0)
				TotUse+=t&0xffff;
			else
				nTmpMenu--;
		}
...
int CMenuBar::UpdateDrawnGrayed(CMenuEx *pMenu, int iMed, int iRange)
{
	int i,n;
	UINT uId;
	if(!pMenu) return 0;
	...
	for (i=0;i<n;i++)
	{
		...
		if (pTmpMenu) 
		{
			UpdateDrawnGrayed(pTmpMenu,iMed,iRange);
		}
		else
		{
			uId=pMenu->GetMenuItemID(i);
			if (uId==ID_SEPARATOR) 
			{
				pMenu->SetFlagsEx(i,0x8000);
				continue;
			}
			UINT t=pMenu->GetFlagsEx(i);
			if ((t&0xffff)!=0xffff)
			{
				WORD tt=(WORD)(t&0xffff);
				if (EvaluateIfGrayed(pMenu,i,tt,iMed,iRange))
					t=((DWORD)tt) | MF_DRAWGRAYED;
				else
					t=(((DWORD)tt)|(t&0xffff0000))&(~MF_DRAWGRAYED);
			}
			else
			{
				t&=(~MF_DRAWGRAYED);
...
BOOL CMenuBar::RefreshDrawnGrayed(CMenuEx *pMenu/*,BOOL &bNoneGrayed*/)
{
	int i,n;
	UINT uID;
	BOOL bAllGrayed=TRUE;
	BOOL bNoneGrayed=TRUE;
	BOOL bGrayed;
	CMenuEx* pSubMenu;
	if(!pMenu || !m_bEnableChange) return FALSE;
	n=pMenu->GetMenuItemCount();
	for(i=0;i<n;i++)
	{
		CString str;
		pMenu->GetMenuText(i,str);
		pSubMenu=pMenu->GetSubMenu(i);
		if(pSubMenu)
			bGrayed=RefreshDrawnGrayed(pSubMenu);
		else
			bGrayed=(pMenu->GetFlagsEx(i))&MF_DRAWGRAYED;
		if (bGrayed) 
		{
			pMenu->SetFlagsEx(i,pMenu->GetFlagsEx(i)|MF_DRAWGRAYED);
			bNoneGrayed=FALSE;
		}
		else
		{
	...
	if (bNoneGrayed) 
		pMenu->SetFlagsEx(n,pMenu->GetFlagsEx(n)|MF_DRAWGRAYED);
	else
		pMenu->SetFlagsEx(n,pMenu->GetFlagsEx(n)&(~MF_DRAWGRAYED));
	return bAllGrayed;
}

void CMenuBar::ResetMenuUsages(CMenuEx *pMenu)
{
	int i,n;
	UINT uId;
	if(!pMenu) return;
	...
	for (i=0;i<n;i++)
	{
		...
		if (pTmpMenu) 
		{
			ResetMenuUsages(pTmpMenu);
		}
		else
		{
			uId=pMenu->GetMenuItemID(i);
			pMenu->SetFlagsEx(i,(pMenu->GetFlagsEx(i)&0xffff0000)|0x8000);
		}
	}
	return;
}

void CMenuBar::UpdateSeparatorGrayed(CMenuEx *pMenu)
{
	int i,n;
	UINT uId;
	CMenuEx* pSubMenu;
	...
	for (i=0;i<n;i++)
	{
		...
		else 
		{
			...
			if(pSubMenu)
			{
				UpdateSeparatorGrayed(pSubMenu);
				bSep=FALSE;
			}
			else
			{
				...
				{
					if (bSep)
						pMenu->SetFlagsEx(i,0x8000 | MF_DRAWGRAYED);
					else 
						bSep=TRUE;
					continue;
				}
...
void CMenuBar::EnableGrayedDraw(BOOL a, BOOL b)
{
	m_bUpdating=TRUE;
	if(m_pMenu) m_pMenu->EnableGrayedDraw(a,b);
	m_bUpdating=FALSE;
}

void CMenuBar::RefreshMenuPainting(CMenuEx *pMenu,int nIndex)
{
	int i,n,a;
	if (nIndex==0 || !pMenu->IsEnableArrows()) return;
	if (nIndex<0)
	{
		a=0;
		n=pMenu->GetMenuItemCount();
	...
	for (i=a;i<n;i++)
	{
		...
		RefreshDrawnGrayed(pSubMenu);
		UpdateSeparatorGrayed(pSubMenu);
	}
}

BOOL CMenuBar::ReOpenMenu(int nLevels)
{
	int i;
	for(i=2;i<nLevels;i++)
	{
		...
		for (;a<=b;a++)
		{
			if((m_arrSel[i-1])->GetMenuItemID(a)!=ID_SEPARATOR)
...
CFrameWnd* CMenuBar::GetParentFrame()
{
	if (m_pParent==NULL)
		return CControlBar::GetParentFrame();
	else
		return (CFrameWnd*)m_pParent;
}

void CMenuBar::SetIntelligentMode(BOOL bMode)
{
	m_bMode=bMode;
	if (!bMode)
	{
		m_pMenu->EnableGrayedDraw(TRUE);
		m_pMenu->EnableArrows(FALSE);
		m_pMenu->UpdateGrayedOnly(m_pMenu,TRUE);
	}
	else
	{
		m_pMenu->EnableGrayedDraw(FALSE);
		m_pMenu->EnableArrows(TRUE);
		m_pMenu->UpdateGrayedOnly(m_pMenu,FALSE);
	}
	UpdateBar();
}

WORD CMenuBar::IncMenuUsage(CMenuEx *pParent, UINT pos, WORD curusage)
{
	// This function is called to increment current usage of menues.
	// Override it if you want to increment menu usage depending on special features.
	
	// P.S. Uou can use pParent->GetExtraDataPtr(pos) to obtain extra data defined 
	// in your application
	return curusage+1;
}

BOOL CMenuBar::EvaluateIfGrayed(CMenuEx *pParent, UINT pos, WORD &curusage, int iMed, int iRange)
{
	// This function is called to determine if a menu must be painted 
	// grayed or not. 
	// This function can also modify current usage of menues.
	// Override it if you want to change beaviour depending on special features.

	// P.S. Uou can use pParent->GetExtraDataPtr(pos) to obtain extra data defined 
	// in your application
	int oldusage=curusage;
	curusage-=iMed+0x8000;
	if (curusage<0) curusage=0;
	if (oldusage <(iMed - iRange))
		return TRUE;
	else
		return FALSE;
}

void CMenuBar::SetParent(CWnd *pParent)
{
	m_pParent=pParent;
}

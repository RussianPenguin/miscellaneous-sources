...
BOOL CTickTestApp::InitInstance()
{
  ...
  if (AfxMessageBox(_T("This program will appear to hang for up to 20 seconds while determining CPU speed\nThis is normal behaviour, Continue?"), MB_YESNO) == IDYES)
  {
    ...
    //Display the results
    CString sMsg;
    sMsg.Format(_T("Sleep(1000) really sleeped for %f seconds and the computer has been running for %f seconds"), tick2.GetTickCountAsSeconds() - tick1.GetTickCountAsSeconds(), tick2.GetTickCountAsSeconds());
    AfxMessageBox(sMsg);
  }
